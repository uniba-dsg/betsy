...
public class ConsoleTasks {
    ...
    public static void setupAnt(Path antPath) {
        ...
        ConsoleTasks.executeOnUnixAndIgnoreError(CliCommand.build(binFolder, "chmod").values("+x", binFolder.resolve("ant").toString()));
        ConsoleTasks.executeOnUnix(CliCommand.build("sync"));
    }

    public static void setupMvn(Path mvnPath) {
        Path binFolder = mvnPath.toAbsolutePath();
        ConsoleTasks.executeOnUnixAndIgnoreError(CliCommand.build(binFolder, "chmod").values("+x", binFolder.resolve("mvn").toString()));
        ConsoleTasks.executeOnUnix(CliCommand.build("sync"));
    }
