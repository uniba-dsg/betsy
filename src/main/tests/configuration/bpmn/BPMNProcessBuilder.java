...
public class BPMNProcessBuilder {
    ...
    public static EngineIndependentProcess buildBasicProcess(String name, String description, Feature feature, BPMNTestCase... testCases) {
        return new EngineIndependentProcess(ROOT_FOLDER.resolve("basics").resolve(name + FileTypes.BPMN), description, Arrays.asList(testCases), feature, partners);
    }

    public static EngineIndependentProcess buildDataProcess(String description, Feature feature, BPMNTestCase... testCases) {
        return new EngineIndependentProcess(ROOT_FOLDER.resolve("data").resolve(feature.getName() + FileTypes.BPMN), description, Arrays.asList(testCases), feature, partners);
    }

    public static EngineIndependentProcess buildPatternProcess (String description, Feature feature, BPMNTestCase... testCases){
        return new EngineIndependentProcess(ROOT_FOLDER.resolve("cfpatterns").resolve(feature.getName() + FileTypes.BPMN), description, Arrays.asList(testCases), feature, partners);
    }
