...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="is-default-message-exchange-used">
        ...
        <xsl:param name="activity"
                   select="ancestor-or-self::*[self::bpel:process or self::bpel:onEvent or self::bpel:forEach[@parallel='yes']][1]"/>
    ...
    <xsl:template name="default-message-exchange-name">
        <xsl:param name="scope"
                   select="ancestor-or-self::*[self::bpel:process or self::bpel:scope[parent::bpel:onEvent or parent::bpel:forEach[@parallel='yes']]][1]"/>
    ...
    <xsl:template
            match="bpel:process/bpel:messageExchanges | bpel:onEvent/bpel:scope/bpel:messageExchanges | bpel:forEach[@parallel='yes']/bpel:scope/bpel:messageExchanges">
    ...
    <xsl:template
            match="bpel:onEvent/bpel:scope[not(bpel:messageExchanges)] | bpel:forEach[@parallel='yes']/bpel:scope[not(bpel:messageExchanges)]">
        <xsl:variable name="is-default-message-exchange-used">
            <xsl:call-template name="is-default-message-exchange-used">
                <xsl:with-param name="activity" select=".."/>
            </xsl:call-template>
    ...
    <xsl:template match="bpel:reply[not(@messageExchange)]">
        <xsl:copy>
    ...
    <xsl:template match="bpel:onMessage[not(@messageExchange)] | bpel:receive[not(@messageExchange)]">
        <xsl:variable name="myRole" select="true()"/>
        ...
        <xsl:variable name="partnerLinkNamespace"
                      select="namespace::*[local-name() = substring-before($partnerLink/@partnerLinkType, ':')]"/>
        <xsl:variable name="definitions"
                      select="document(/bpel:process/bpel:import[@importType='http://schemas.xmlsoap.org/wsdl/']/@location)/wsdl:definitions[@targetNamespace=$partnerLinkNamespace]"/>
        <xsl:variable name="portType">
            <xsl:choose>
                <xsl:when test="$myRole">
                    <xsl:value-of
                            select="substring-after($definitions/plnk:partnerLinkType[@name=substring-after($partnerLink/@partnerLinkType, ':')]/plnk:role[@name=$partnerLink/@myRole]/@portType, ':')"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of
                            select="substring-after($definitions/plnk:partnerLinkType[@name=substring-after($partnerLink/@partnerLinkType, ':')]/plnk:role[@name=$partnerLink/@partnerRole]/@portType, ':')"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="is-request-response"
                      select="$definitions/wsdl:portType[@name=$portType]/wsdl:operation[@name=$operation]/wsdl:output"/>

        <xsl:choose>
            <xsl:when test="$is-request-response">
                <xsl:copy>
                    <xsl:attribute name="messageExchange">
                        <xsl:call-template name="default-message-exchange-name"/>
                    </xsl:attribute>
